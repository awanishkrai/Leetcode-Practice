...
//recr(i,j)=min no of moves cut rectangle size(i,j) into squares
// Solve function for each test case
vector<vector<int>>memo;
int recr(int i,int j){
    if(i==j)return 0;
    if(memo[i][j]!=-1)return memo[i][j];
    int moves=INT_MAX;
    if(i>1){
        for(int k=1;k<=i/2;k++){
            moves=min(moves,1+recr(k,j)+recr(i-k,j));
        }
    }
    if(j>1){
         for(int k=1;k<=j/2;k++){
            moves=min(moves,1+recr(i,k)+recr(i,j-k));
        }
