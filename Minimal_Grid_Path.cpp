...
//recr(i,j)= lexo smallest string starting from i,j;

string recr(int i,int j,vector<string>&a,vector<vector<string>>&dp){
    int n=a.size();
    if(i>=n||j>=n)return "{";
    if(i==n-1&&j==n-1)return string(1,a[i][j]);
    if(dp[i][j]!="#")return dp[i][j];
    return dp[i][j]=a[i][j]+min(recr(i+1,j,a,dp),recr(i,j+1,a,dp));
}
